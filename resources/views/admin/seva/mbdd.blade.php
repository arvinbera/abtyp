...
@section('header')
<title>ABTYP MBDD List | Admin Dashboard</title>
          ...
          <div class="col-12">
            ...
            <div class="card">
              <div class="card-header">
                <!-- ===========================Filter Start ================================== -->
                 <form method="post" action="{{ route('admin.mbdd-report-filter') }}">
                @csrf
             ...
             <div class="table-responsive">
              ...
              <div class="card-body">
                 <form method="post"  action="{{ route('admin.mbdd_pdf_submit',['download'=>'pdf']) }}">
                    @csrf
               <table id="example" class="display nowrap" style="width:100%">
                 ...
                 <tr>
                   <td>
                    ...
                    <input type="checkbox" name="atdccb[]" value="{{$mbdd->id}}" id="checkAll" multiple="">
                  </td>
                    <td>{{ $loop->iteration }}</td>
                    <td>{{ $mbdd->username }}</td>
                    <td><?php $newDateFormat2 = date('M/Y', strtotime( $mbdd->monthly_report_month )); echo $newDateFormat2; ?>
                        
                     </td>
                    <td>{{ $mbdd->address }}</td>
                    <td>{{ $mbdd->name_of_camps  }}</td>
                    <td>{{ $mbdd->date   }}</td>
                    <td>{{ $mbdd->time }}</td>
                    <td>{{ $mbdd->venue }}</td>
                    ...
                    <td>{{ $mbdd->guests }}</td>
                    <td>{{ $mbdd->total }}</td>
                    <td>{{ $mbdd->mbdd_prepared_by }}</td>
                    <td><a href="{{ route('admin-mbdd-pdfview',['id'=>$mbdd->id, 'monthly_report_month'=>$mbdd->monthly_report_month,'download'=>'pdf']) }}"><i class="fas fa-download"></i></a></td>
                     <td><a href="{{url('admin/mbdd-print')}}/{{$mbdd->id}}" target="_blank"><i class="fas fa-print"></i></a></td>
