   ...
   <!--               <form method="post" action="{{ route('user.monthly-report_filer') }}">
                ...
                <div class="row">
                        ...
                        <div class="col-12">
                            <div class="card">
									<div class="card-header">
										<div class="card-title">Yuva Sangam Monthly Report</div>
									</div>
									<div class="card-body">
                  ...
                  <tr>
                    ...
                    <th class="wd-15p border-bottom-0">TYP (New Members added this month)</th>
                    <th class="wd-15p border-bottom-0">TKM (New Members added this month)</th>
                    <th class="wd-15p border-bottom-0">Current Total Strength</th>
                    <th class="wd-15p border-bottom-0">Special Thanks To</th>
                    <th class="wd-15p border-bottom-0">Brief Report</th>
                    <th class="wd-15p border-bottom-0">New Members list updated on Yuva Sangam</th>
                 ...
                 <tr>
                    ...
                    <td>{{ $monthyreport->typ_ys_no_new_members }}</td>
                    <td>{{ $monthyreport->tkm_ys_no_new_members }}</td>
                    <td>{{ $monthyreport->ys_conveynor }}</td>
                    <td>{{ $monthyreport->ys_special_thanks_to }}</td>
                    <td>{{ $monthyreport->ys_brief_reports }}</td>
                    <td>{{ $monthyreport->new_member_list_updated_on_ys }}</td>
                    ...
                    <td>{{ $monthyreport->filled_by_role }}</td>
                      <td>
                        @if($monthyreport->status == 'pending')
                         <a href="{{url('branch/ys-status/'.$monthyreport->id)}}"><button class="btn btn-warning w-100">Pending</button></a>
                        @else
                        <a href="#"><button class="btn btn-success w-100">Accept</button></a>
                        @endif
                    </td>
                    <td><a href="{{url('branch/yuvasangam_pdfview')}}/{{$monthyreport->id}}/{download}"><button class="btn btn-success w-100"><i class="fa fa-file-pdf-o" aria-hidden="true"></i> Pdf</button></a></td>
                    <td><a href="{{url('branch/yuvasangam_print')}}/{{$monthyreport->id}}" target="_blank"><button class="btn btn-info w-100"><i class="fa fa-print" aria-hidden="true"></i> Print</button></td>
