   ...
   <!--               <form method="post" action="{{ route('user.monthly-report_filer') }}">
                ...
                <div class="row">
                        ...
                        <div class="col-12">
                            <div class="card">
									<div class="card-header">
										<div class="card-title">JSV Monthly Report</div>
									</div>
									<div class="card-body">
                  ...
                  <tr>
                    ...
                    <th class="wd-15p border-bottom-0">Sanskarak Name</th>
                 ...
                 <tr>
                    ...
                    <td>{{ $monthyreport->jsv_date }}</td>
                    <td>{{ $monthyreport->jsv_time }}</td>
                    <td>{{ $monthyreport->jsv_venue }}</td>
                    <td>{{ $monthyreport->jsv_sanskar_name }}</td>
                    <td>{{ $monthyreport->jsv_no_of_participant }}</td>
                    <td>{{ $monthyreport->jsv_convenors }}</td>
                    <td>{{ $monthyreport->jsv_key_member }}</td>
                    <td>{{ $monthyreport->jsv_sponsors }}</td>
                    <td>{{ $monthyreport->jsv_specila_thanks_to }}</td>
                    <td>{{ $monthyreport->jsv_brief_report }}</td>
                    <td>{{ $monthyreport->jsv_chaitra_aatma }}</td>
                    <td>{{ $monthyreport->jsv_abtyp_members }}</td>
                    <td>{{ $monthyreport->jsv_chief_guest }}</td>
                    <td>{{ $monthyreport->jsv_guest }}</td>

                    <td>{{ $monthyreport->filled_by }}</td>
                    <td>{{ $monthyreport->filled_by_role }}</td>
                        <td>
                        @if($monthyreport->status == 'pending')
                         <a href="{{url('branch/jsv-status/'.$monthyreport->id)}}"><button class="btn btn-warning w-100">Pending</button></a>
                        @else
                        <a href="#"><button class="btn btn-success w-100">Accept</button></a>
                        @endif
                    </td>
                    
                    <td><a href="{{url('branch/jsv_pdfview')}}/{{$monthyreport->id}}/{download}"><button class="btn btn-success w-100"><i class="fa fa-file-pdf-o" aria-hidden="true"></i> Pdf</button></a></td>
                    <td><a href="{{url('branch/jsv_print')}}/{{$monthyreport->id}}" target="_blank"><button class="btn btn-info w-100"><i class="fa fa-print" aria-hidden="true"></i> Print</button></td>
