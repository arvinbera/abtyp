   ...
   <!--               <form method="post" action="{{ route('user.monthly-report_filer') }}">
                ...
                <div class="row">
                        ...
                        <div class="col-12">
                            <div class="card">
									<div class="card-header">
										<div class="card-title">JVK Monthly Report</div>
									</div>
									<div class="card-body">
                 ...
                 <tr>
                    ...
                    <td>{{ $monthyreport->jvk_date }}</td>
                    <td>{{ $monthyreport->jvk_time }}</td>
                    <td>{{ $monthyreport->jvk_venue }}</td>
                    <td>{{ $monthyreport->jvk_in_association }}</td>
                    <td>{{ $monthyreport->jvk_teachers_name }}</td>
                    <td>{{ $monthyreport->jvk_no_of_participants }}</td>
                    <td>{{ $monthyreport->jvk_convenors }}</td>
                    <td>{{ $monthyreport->jvk_key_members }}</td>
                    <td>{{ $monthyreport->jvk_sponsor }}</td>
                    <td>{{ $monthyreport->jvk_special_thanks_to }}</td>
                    <td>{{ $monthyreport->jvk_chaitra_aatma }}</td>
                    <td>{{ $monthyreport->jvk_abtyp_members }}</td>
                    <td>{{ $monthyreport->jvk_chief_guest }}</td>
                    <td>{{ $monthyreport->jvk_guest }}</td>
                  ...
                  <td>
                        @if($monthyreport->status == 'pending')
                         <a href="{{url('branch/jvk-status/'.$monthyreport->id)}}"><button class="btn btn-warning w-100">Pending</button></a>
                        @else
                        <a href="#"><button class="btn btn-success w-100">Accept</button></a>
                        @endif
                    </td>
                    <td><a href="{{url('branch/jvk_pdfview')}}/{{$monthyreport->id}}/{download}"><button class="btn btn-success w-100"><i class="fa fa-file-pdf-o" aria-hidden="true"></i> Pdf</button></a></td>
                    <td><a href="{{url('branch/jvk_print')}}/{{$monthyreport->id}}" target="_blank"><button class="btn btn-info w-100"><i class="fa fa-print" aria-hidden="true"></i> Print</button></td>
