   ...
   <!--               <form method="post" action="{{ route('user.monthly-report_filer') }}">
                ...
                <div class="row">
                        ...
                        <div class="col-12">
                            <div class="card">
									<div class="card-header">
										<div class="card-title">Barah Vrat Monthly Report</div>
									</div>
									<div class="card-body">
                  ...
                  <tr>
                    ...
                    <th class="wd-15p border-bottom-0">Speaker's Name</th>
                 ...
                 <tr>
                    ...
                    <td>{{ $monthyreport->bv_venue }}</td>
                    <td>{{ $monthyreport->bv_in_association }}</td>
                    <td>{{ $monthyreport->bv_sanskar_name }}</td>
                    <td>{{ $monthyreport->bv_no_of_participant }}</td>
                    <td>{{ $monthyreport->bv_convenors }}</td>
                    <td>{{ $monthyreport->bv_key_members }}</td>
                    <td>{{ $monthyreport->bv_sponsors }}</td>
                    <td>{{ $monthyreport->bv_special_thanks_to }}</td>
                    <td>{{ $monthyreport->bv_brief_report }}</td>
                    <td>{{ $monthyreport->bv_abtyp_members }}</td>
                    <td>{{ $monthyreport->bv_chief_guest }}</td>
                    <td>{{ $monthyreport->bv_guets }}</td>
                   ...
                   <td>
                        @if($monthyreport->status == 'pending')
                         <a href="{{url('branch/bv-status/'.$monthyreport->id)}}"><button class="btn btn-warning w-100">Pending</button></a>
                        @else
                        <a href="#"><button class="btn btn-success w-100">Accept</button></a>
                        @endif
                    </td>
                    <td><a href="{{url('branch/barah_varat_pdfview')}}/{{$monthyreport->id}}/{download}"><button class="btn btn-success w-100"><i class="fa fa-file-pdf-o" aria-hidden="true"></i> Pdf</button></a></td>
