   ...
   <!--               <form method="post" action="{{ route('user.monthly-report_filer') }}">
                ...
                <div class="row">
                        ...
                        <div class="col-12">
                            <div class="card">
                                    <div class="card-header">
                                        <div class="card-title">TKM Monthly Report</div>
                                    </div>
                                    <div class="card-body">
                  ...
                  <tr>
                    ...
                    <th class="wd-15p border-bottom-0">Event Title</th>
                     ...
                     <th class="wd-15p border-bottom-0">Special Thanks</th>
                      <th class="wd-15p border-bottom-0">Brief Report</th>
                       <th class="wd-15p border-bottom-0">Chaitra Aatma</th>
                        <th class="wd-15p border-bottom-0">ABTYP / BLUE BRIGADE</th>
                    ...
                    <th class="wd-15p border-bottom-0">statusbranch</th>
                 ...
                 <tr>
                    ...
                    <td>{{ $monthyreport->tkm_name }}</td>
                    <td>{{ $monthyreport->tkm_time }}</td>
                    <td>{{ $monthyreport->tkm_event_title }}</td>
                    <td>{{ $monthyreport->tkm_in_association }}</td>
                    <td>{{ $monthyreport->tkm_no_of_participants }}</td>
                    <td>{{ $monthyreport->tkm_convenors }}</td>
                    <td>{{ $monthyreport->tkm_key_members }}</td>
                    <td>{{ $monthyreport->tkm_sponsors }}</td>
                    <td>{{ $monthyreport->tkm_special_thanks_to }}</td>
                    <td>{{ $monthyreport->tkm_brief_report }}</td>
                    <td>{{ $monthyreport->tkm_chaitra_aatma }}</td>
                    <td>{{ $monthyreport->tkm_abtyp_members }}</td>
                    <td>{{ $monthyreport->tkm_chief_guest }}</td>
                    <td>{{ $monthyreport->tkm_guest }}</td>
                    ...
                    <td>{{ $monthyreport->filled_by_role }}</td>
                        ...
                        @if($monthyreport->status == 'pending')
                         <a href="{{url('branch/tkm-status/'.$monthyreport->id)}}"><button class="btn btn-warning w-100">Pending</button></a>
                        @else
                        <a href="#"><button class="btn btn-success w-100">Accept</button></a>
                        @endif
                    </td>
                    <td><a href="{{url('branch/tkm_pdfview')}}/{{$monthyreport->id}}/{download}"><button class="btn btn-success w-100"><i class="fa fa-file-pdf-o" aria-hidden="true"></i> Pdf</button></a></td>
                    <td><a href="{{url('branch/ttk_print')}}/{{$monthyreport->id}}" target="_blank"><button class="btn btn-info w-100"><i class="fa fa-print" aria-hidden="true"></i> Print</button></td>
